...
def enigma_v0_1(text, ref, rot1, rot2, rot3):
    text = ''.join([i for i in text.upper() if i.isalpha()])
    rot_list = [rot3, rot2, rot1]

    for rot in rot_list:
        text = ''.join([rotor(x, rot, False) for x in text])
    
    text = ''.join([reflector(x, ref) for x in text])

    rot_list.reverse()

    for rot in rot_list:
        text = ''.join([rotor(x, rot, True) for x in text])
...
def enigma_v0_2(text, ref, rot1, shift1, rot2, shift2, rot3, shift3):
    text = ''.join([i for i in text.upper() if i.isalpha()])
    rot_list = [rot3, rot2, rot1]
    ...
    for i in range(len(rot_list)):
        ...
        text = ''.join([shift_symbol(x, shift) for x in text])
        last_shift = shift_list[i]

        text = ''.join([rotor(x, rot_list[i], False) for x in text])

    text = ''.join([shift_symbol(x, -last_shift) for x in text])
    last_shift = 0
    ...
    for i in range(len(rot_list)):
        ...
        text = ''.join([rotor(x, rot_list[i], True) for x in text])
...
def enigma_v0_3(text, ref, rot1, shift1, rot2, shift2, rot3, shift3):
    text = ''.join([i for i in text.upper() if i.isalpha()])
    rot_list = [rot3, rot2, rot1]
    shift_list = [shift3, shift2, shift1]
    last_shift = 0
